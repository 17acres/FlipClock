...
typedef struct SPITivaDMA_Object {
    ti_sysbios_knl_Semaphore_Struct      transferComplete;     
...
void spiCallback(SPI_Handle handle, SPI_Transaction *transaction) {
	...
	xdc_runtime_System_printf__E("SPI Callback: Frame index %d, status %d", (uint32_t) transaction->arg, transaction->status);
}
